...
<body>
    <form action="validacionPhp.php" method="POST" id="login" onsubmit="return comprobarForm()">
        ...
        <?php if(isset($_GET['userVacio'])){ echo 'Usuario vacio. Debes introducir un usuario valido.</br>'; } ?>
        <?php if(isset($_GET['userError'])){ echo 'User no es valido. Solo permite letras.</br>'; } ?>
        ...
        <?php if(isset($_GET['pwdVacio'])){ echo 'Contrasena vacia. Debes introducir una contrasena valida.</br>'; } ?>
        <?php if(isset($_GET['pwdError'])){ echo 'La contrasena no es valido. Debes introducir una contrasena valida.</br>'; } ?>
        <input type="submit" name="enter" value="Veridicar">
    </form>
